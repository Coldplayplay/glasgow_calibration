...
{
    opencvStr = "opencv"; // checkboard target
    // scope for other calibration targets
}

// int cam should be -1 for mono configuration, 0 for left and 1 for right (stereo configurations)
bool Calibration::runCalibrationAndSave(Mat&  cameraMatrix, Mat& distCoeffs, vector<vector<Point2f> > imagePoints, int cam)
{
    ...
    {
        if(cam == -1)
            saveCameraParams("camera_calibration.xml", cameraMatrix, distCoeffs, reprojErrs, cal_2Dpoints, totalAvgErr);
...
bool Calibration::runExtrinsic(Mat cameraMatrix, Mat distCoeffs, vector<Point2f> imagePoints)
{
    ...
    {
        ROS_WARN_STREAM("Size object points: " << objectPoints.size());
        ROS_WARN_STREAM("Size image points: " << imagePoints.size());
        ROS_ERROR_STREAM(e.what());
        return false;
    }
...
void Calibration::calibTransform(string outpur_dir, const Mat& in, string labelX2Y)
{
    string output_file = outpur_dir;
    FileStorage fs(output_file, FileStorage::WRITE );
    ROS_INFO_STREAM("Saving calibration transformation at: " << output_file);
    ...
    string name = labelX2Y;
    fs << name << in;
    
}

void Calibration::saveHandEyeTransform(string output_file, const Mat& left, string hName)
{
    // Save stuff
    output_file = calibOutputDir + output_file;
...
//                                  CV_CALIB_FIX_INTRINSIC + CV_CALIB_FIX_ASPECT_RATIO + CV_CALIB_ZERO_TANGENT_DIST +
//                                  CV_CALIB_FIX_K3 + CV_CALIB_FIX_K4 + CV_CALIB_FIX_K5);
